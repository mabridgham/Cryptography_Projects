...
import itertools, re
import vigenereCipherMB, pyperclip, freqAnalysisMB, detectEnglishMB
...
def findRepeatSequencesSpacings(message):
    ...
    for seqLen in range(3,6):
        for seqStart in range(len(message) - seqLen):
            ...
            # Determine what the squence is and store it in seq:
...
def getUsefulFactors(num):
    ...
    # When finding factors, you only need to check the intergers up to
    # MAX_KEY_LENGTH:
...
def getMostCommonFactors(seqFactors):
    ...
    # 18, 23, 36,46,69,92,138,207], 'ALW': [2, 4, 6, ...], ...}.
...
def getNthSubkeysLetters(nth, keyLength, message):
    ...
    # getNthSubkeysLetters(1, 5, 'ABCDEFGI') returns 'AF'
...
def attemptHackWithKeyLength(ciphertext, mostLikelyKeyLength):
    ...
    for nth in range(1, mostLikelyKeyLength +1):
        ...
        # See the englishFreqMatchScore() comments in freqAnalysisMB.py

        freqScores = []
        for possibleKey in LETTERS:
            decryptedText = vigenereCipherMB.decryptMessage(possibleKey, nthLetters)
            keyAndFreqMatchTuple = (possibleKey, freqAnalysisMB.englishFreqMatchScore(decryptedText))
    ...
    for indexes in itertools.product(range(NUM_MOST_FREQ_LETTERS), repeat=mostLikelyKeyLength):
        ...
        decryptedText = vigenereCipherMB.decryptMessage(possibleKey, ciphertextUp)

        if detectEnglishMB.isEnglish(decryptedText):

            # Set the hacked ciphertext to the original
